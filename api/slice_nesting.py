...
def slice_nesting(polygons, container_max_x, iteration_number=1):
    ...
    while (! p2 ):
        ...
        placed_list.append(tuple(new_poly.x, new_poly.y+height))
...
def boundary_check_pass(p1, p2):
    ...
    if (p1.x - p2.x > m and p2.x+p2.width - p1.x > m ):
        if (p1.y - p2.y > and p2.y+py.height - p1.y > m):
            return bbox_lists_check_pass(p1.bbox_list, p2.bbox_list)
        else:
            return True

    else if (p2.x - p1.x > m and p1.x+p1.width - p2.x > m ):
...
def bbox_lists_check_pass(list1, list2):
    ...
    for (box1 in list1):               # TODO: maybe quadtree for better time
        for (box2 in list2):
